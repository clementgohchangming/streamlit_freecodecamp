...
from utils import format_listings
...
def load_data():
    ...
    coins = {}
    coin_data: Dict[Any, Any] = json.loads(data.contents[0])
    listings: List[Dict[str, Any]] = format_listings(coin_data)
    ...
    for i in listings:
        ...
        price.append(i[f"quote.{currency_price_unit}.price"])
        percent_change_1h.append(i[f"quote.{currency_price_unit}.percentChange1h"])
        percent_change_24h.append(i[f"quote.{currency_price_unit}.percentChange24h"])
        percent_change_7d.append(i[f"quote.{currency_price_unit}.percentChange7d"])
        market_cap.append(i[f"quote.{currency_price_unit}.marketCap"])
        volume_24h.append(i[f"quote.{currency_price_unit}.volume24h"])
...
if percent_timeframe == "7d":
    ...
    df_change["percentChange7d"].plot(
        kind="barh",
        color=df_change.positive_percentChange7d.map({True: "g", False: "r"}),
    )
    col3.pyplot(plt)
    ...
    df_change["percentChange24h"].plot(
        kind="barh",
        color=df_change.positive_percentChange24h.map({True: "g", False: "r"}),
    )
    col3.pyplot(plt)
else:
    if sort_values == "Yes":
    ...
    df_change["percentChange1h"].plot(
        kind="barh",
        color=df_change.positive_percentChange1h.map({True: "g", False: "r"}),
    )
    col3.pyplot(plt)
